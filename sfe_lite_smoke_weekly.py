...
def usage():
    ...
    print('')
    print('  python ' + sys.argv[0] + ' SFE-Lite Continuous-Integration-Master')
    print('')
    print('  (' + str(sys.argv) + ')')
    print('')
...
###
###
def get_smoke_test_result(url):
    print('get_smoke_test_result, url: ' + url)
...
def save_test_case_list():
    ...
    now = datetime.now()
    folder = 'SFE-Lite-smoke/' + now.strftime("%Y-%m-%d-%H-%M")
    path = web_server_path + folder
    filename = 'flaky_smoke.html'
...
for x in dirs:
    ...
    get_smoke_test_result(url + x + 'test_cases_smoke.html')
...
if len(sort_test_case_list) > 0:
    body = 'Last 7 days we have flaky smoke test: ' + '<a href=\"' + result_url + '\" />' + NEW_LINE
   
    body += NEW_LINE
    body += BOLD + 'No smoke test should fail on master' + BOLD_RESET + NEW_LINE
else:
    body = 'Awesome! No flaky smoke test the last 7 days on master' + NEW_LINE
...
subject = 'SFE-Lite Smoke test weekly ' + my_date 
